...
async function connectJob(job) {
  const { state, saveCreds } =
    await useMultiFileAuthState(job.dir)
  ...
  sock.ev.on('connection.update', async update => {
    ...
    if (connection === 'connecting') job.status = 'connecting'
    ...
    if (
      ...
      try {
        const code = await sock.requestPairingCode(job.number)
    ...
    if (connection === 'open') {
      ...
      try {
        ...
        try {
          ...
          job.sendError = error?.message || 'Could not send session'
        }
      } catch (error) {
        job.status = 'error'
    ...
    if (connection === 'close') {
      ...
      if (job.status === 'ready' || job.status === 'connected') return
